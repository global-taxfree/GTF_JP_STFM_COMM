...
using GTF_STFM_COMM.Properties;
            ...
            try {
                ...
                if (m_bStart && !CHK_OFFLINE.Checked)
                {
                    ...
                    if (jsonRsult != null && "S".Equals(jsonRsult["login"].ToString()))
                    {
                        ...
                        Boolean CustomSendColumn = Constants.LDB_MANAGER.ExitsTableColumn("MERCHANT", "SEND_CUSTOM_FLAG");

                        if (!CustomSendColumn)
                        {
                            Constants.LDB_MANAGER.alterTable("MERCHANT", "SEND_CUSTOM_FLAG");
                        }

                        Boolean PermitNoColumn = Constants.LDB_MANAGER.ExitsTableColumn("REFUNDSLIP", "PERMIT_NO");

                        if (!PermitNoColumn)
                        {
                            Constants.LDB_MANAGER.alterTable("REFUNDSLIP", "PERMIT_NO");
                        }

                        Boolean NoteColumn = Constants.LDB_MANAGER.ExitsTableColumn("REFUNDSLIP", "REFUND_NOTE");

                        if (!NoteColumn)
                        {
                            Constants.LDB_MANAGER.alterTable("REFUNDSLIP", "REFUND_NOTE");
                        }
